...
//CREATED BY WELL
std::string TraCICommandInterface::Vehicle::getVehicleId()
{
    return nodeId;
}

//CREATED BY WELL
std::list<std::string> TraCICommandInterface::Vehicle::getVehiclesIds()
{
    return traci->genericGetStringList(CMD_GET_VEHICLE_VARIABLE, "", ID_LIST, RESPONSE_GET_VEHICLE_VARIABLE);
}

//CREATED BY WELL
Coord TraCICommandInterface::Vehicle::getVehiclePosition()
{
...
//CREATED BY WELL
Coord TraCICommandInterface::convertPositionSumo2Veins(double inputX, double inputY, int rsuID = 0){
    Coord resultado = connection.traci2omnet(TraCICoord(inputX, inputY));
    std::cout << "*.rsu[" << rsuID << "].mobility.x = " << resultado.x << endl;
    std::cout << "*.rsu[" << rsuID << "].mobility.y = " << resultado.y << endl;
    std::cout << "*.rsu[" << rsuID << "].mobility.z = 3\n" << endl;
    return resultado;
}
...
std::string TraCICommandInterface::Person::getRoadId()
{
    return traci->genericGetString(CMD_GET_PERSON_VARIABLE, nodeId, VAR_ROAD_ID, RESPONSE_GET_PERSON_VARIABLE);
}

double TraCICommandInterface::Person::getAngle()
{
    return traci->genericGetDouble(CMD_GET_PERSON_VARIABLE, nodeId, VAR_ANGLE, RESPONSE_GET_PERSON_VARIABLE);
}

double TraCICommandInterface::Person::getSpeed()
{
    return traci->genericGetDouble(CMD_GET_PERSON_VARIABLE, nodeId, VAR_SPEED, RESPONSE_GET_PERSON_VARIABLE);
}

std::list<std::string> TraCICommandInterface::Person::getPersonIds()
{
    return traci->genericGetStringList(CMD_GET_PERSON_VARIABLE, "", ID_LIST, RESPONSE_GET_PERSON_VARIABLE);
}

int32_t TraCICommandInterface::Person::getCount()
{
    return traci->genericGetInt(CMD_GET_PERSON_VARIABLE, "", ID_COUNT, RESPONSE_GET_PERSON_VARIABLE);
}

Coord TraCICommandInterface::Person::getPosition()
{
    return traci->genericGetCoord(CMD_GET_PERSON_VARIABLE, nodeId, VAR_POSITION, RESPONSE_GET_PERSON_VARIABLE);
}
