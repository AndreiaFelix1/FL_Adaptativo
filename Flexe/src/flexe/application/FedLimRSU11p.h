...
public:
    ...
    int currentRound = 0;
    void logRound(const std::string& message);
...
protected:
    ...
    void logCommunication(std::string node, long msgId, double timestamp, std::string type);
    ...
    std::map<int, std::tuple<std::string, double, Coord>> connectedClientsMap; //(Veins-ID, <SUMO-ID, Local Loss, Position>)
    std::vector<int> connectedClientsVector;
    std::vector<int> receivedModelsVector;
    std::vector<int> receivedNumberExamplesVector;

    std::map<int, double> clientF1Map;
    ...
    struct ClientCommunication {
        ...
        // Qualidade usada para a próxima seleção
        double qualityScore = 0.0;
    };

    std::map<int, ClientCommunication> clientCommunicationMap;

    std::map<long, double> modelSendTimes;
